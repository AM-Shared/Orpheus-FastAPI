...
async def root(request: Request):
    ...
    return templates.TemplateResponse(
        "tts.html", {"request": request, "voices": AVAILABLE_VOICES}
    )
...
async def web_ui(request: Request):
    ...
    return templates.TemplateResponse(
        "tts.html", {"request": request, "voices": AVAILABLE_VOICES, "config": config}
    )
...
async def save_config(request: Request):
    ...
    for key, value in data.items():
        if key in [
            ...
            "ORPHEUS_TEMPERATURE",
            "ORPHEUS_TOP_P",
        ]:  # Removed ORPHEUS_REPETITION_PENALTY since it's hardcoded now
            try:
...
async def generate_from_web(
    request: Request, text: str = Form(...), voice: str = Form(DEFAULT_VOICE)
):
    ...
    if use_batching:
        print(
            f"Using batched generation for long text from web form ({len(text)} characters)"
        )
